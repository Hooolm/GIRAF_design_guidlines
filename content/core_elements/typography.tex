...
\noindent All text should by default be black (\colref{1.1}) across the suite when placed in buttons, views, etc. When creating hint text, e.g. when there are no applications present in the launcher, one should use a gray text-color (\colref{1.2}). When using the showcase library to highlight features, the text color of the help text should be white (\colref{1.5}), so it is readable on the dark background. 

\todo[inline]{Insert a preview of the font}